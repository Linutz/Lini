...
import { Canvas as NativeCanvas, CanvasRenderingContext2D as NativeCanvasRenderingContext2D, FontLibrary, Image as NativeImage, loadImage, Path2D, type Font, type RenderOptions } from 'skia-canvas';
...
export declare type RenderImageFormat = `${`${'image/' | ''}${'png' | 'jpg'}` | 'image/svg+xml' | 'svg' | `${'application/' | ''}pdf`}${`@${number}x` | ''}`;
